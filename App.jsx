...
const DEFAULT_TRACKS = [
  { id: "t1", title: "Off The No Go", artist: "Aurelius Music", duration: 180, coverEmoji: "🔥", genre: "Hip-Hop", url: "https://drive.google.com/uc?export=download&id=1Th0RFRiloYy5irlh2F4kOvDltsx9Q_bs", active: true, plays: 0 },
];
...
// ─── WAVEFORM ─────────────────────────────────────────────────────────────────
function Waveform({ playing, progress, duration, onSeek }) {
  const BARS = 48;
  ...
  const handleClick = (e) => {
    ...
    onSeek && onSeek(((e.clientX - rect.left) / rect.width) * duration);
  };
  return (
    <div ref={ref} onClick={handleClick} style={{ display:"flex", alignItems:"center", gap:2, height:44, cursor:"pointer", padding:"0 2px" }}>
      {Array.from({length:BARS},(_,i) => {
        const filled = i/BARS < pct;
        const h = 10 + Math.sin(i*0.7)*8 + Math.sin(i*0.3)*6;
        return <div key={i} style={{ width:4, borderRadius:2, flexShrink:0, height:`${h}px`, background: filled ? "#FFD700" : "#1C1C24", animation: playing && !filled ? `wvb ${0.4+(i%7)*0.07}s ease-in-out infinite alternate` : "none", animationDelay:`${i*0.02}s`, transition:"background .1s" }}/>;
      })}
    </div>
  );
}
...
export default function AureliusHub() {
  ...
  const [showNewLink,setShowNewLink]=useState(false); const [showNewDrop,setShowNewDrop]=useState(false); const [showNewTrack,setShowNewTrack]=useState(false);
  ...
  useEffect(()=>{
    const loadAll = async () => {
      setLoading(true);
      try {
        const [linksRows, dropsRows, tracksRows, messagesRows, settingsRows] = await Promise.all([
          db.getAll("links"), db.getAll("drops"), db.getAll("tracks"),
          db.getAll("messages"), db.getAll("settings")
        ]);
  ...
  useEffect(()=>{
    ...
    if(playing && currentTrack?.url) audioRef.current.play().catch(()=>{});
    else if(currentTrack?.url) audioRef.current.pause();
  },[playing]);
  ...
  useEffect(()=>{
    ...
    const onEnded=()=>{ if(repeat){audioRef.current.currentTime=0;audioRef.current.play();}else handleNext(); };
    audioRef.current.addEventListener("timeupdate",onTime);
    audioRef.current.addEventListener("ended",onEnded);
    return()=>{ audioRef.current?.removeEventListener("timeupdate",onTime); audioRef.current?.removeEventListener("ended",onEnded); };
  },[repeat,tracks,currentTrack,shuffle]);

  // Simulated progress for tracks with no URL
  useEffect(()=>{
    if(!currentTrack||currentTrack.url||!playing) return;
    const iv=setInterval(()=>setProgress(p=>{ if(p>=currentTrack.duration){handleNext();return 0;} return p+1; }),1000);
  ...
  const playTrack=async(track)=>{
    ...
    setProgress(0); setCurrentTrack(track); setPlaying(true);
  ...
  const handleNext=()=>{
    ...
    const next=shuffle?activeTracks[Math.floor(Math.random()*activeTracks.length)]:activeTracks[(idx+1)%activeTracks.length];
    setProgress(0); setCurrentTrack(next); setPlaying(true);
  };
  const handlePrev=()=>{
    if(!activeTracks.length) return;
    ...
    setProgress(0); setCurrentTrack(prev); setPlaying(true);
  };
  const handleSeek=(val)=>{ setProgress(val); if(audioRef.current) audioRef.current.currentTime=val; };
      ...
      <div style={{maxWidth:520,margin:"0 auto",padding:"16px 16px 50px"}}>
        ...
        {tab==="player" && (
          <div style={{animation:"fadein .3s ease"}}>
            {ct ? (
              <div style={{background:"linear-gradient(135deg,#0C0C14,#0A0A10)",border:"1px solid #FFD70022",borderRadius:20,padding:20,marginBottom:18,textAlign:"center"}}>
                <div style={{width:110,height:110,borderRadius:"50%",background:"linear-gradient(135deg,#1C1C24,#FFD70022)",margin:"0 auto 14px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:48,border:"3px solid #FFD70033",animation:playing?"disc 8s linear infinite":"none",boxShadow:playing?"0 0 30px #FFD70033":"none",transition:"box-shadow .3s"}}>{ct.coverEmoji}</div>
                <div style={{fontFamily:"'Anton'",fontSize:20,letterSpacing:2,marginBottom:2}}>{ct.title}</div>
                <div style={{fontSize:10,color:"#555",letterSpacing:2,marginBottom:16}}>{ct.artist} · {ct.genre}</div>
                <div style={{marginBottom:8}}><Waveform playing={playing} progress={progress} duration={ct.duration} onSeek={handleSeek}/></div>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:"#555",marginBottom:16}}><span>{fmtTime(progress)}</span><span>{fmtTime(ct.duration)}</span></div>
                <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:16,marginBottom:14}}>
                  <button className={`ctrl-btn${shuffle?" on":""}`} onClick={()=>setShuffle(s=>!s)}>
                    <svg width={18} height={18} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M16 3h5v5M4 20L21 3M21 16v5h-5M15 15l6 6M4 4l5 5"/></svg>
                  </button>
                  <button className="ctrl-btn" onClick={handlePrev} style={{fontSize:22}}>⏮</button>
                  <button className="play-main" onClick={()=>setPlaying(p=>!p)}>{playing?"⏸":"▶"}</button>
                  <button className="ctrl-btn" onClick={handleNext} style={{fontSize:22}}>⏭</button>
                  <button className={`ctrl-btn${repeat?" on":""}`} onClick={()=>setRepeat(r=>!r)}>
                    <svg width={18} height={18} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M17 1l4 4-4 4M3 11V9a4 4 0 014-4h14M7 23l-4-4 4-4M21 13v2a4 4 0 01-4 4H3"/></svg>
                  </button>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                  <span style={{fontSize:13}}>🔈</span>
                  <input type="range" min={0} max={1} step={0.01} value={volume} onChange={e=>setVolume(+e.target.value)} style={{flex:1,accentColor:"#FFD700",cursor:"pointer"}}/>
                  <span style={{fontSize:13}}>🔊</span>
                </div>
                <div style={{display:"flex",justifyContent:"center",gap:10}}>
                  <button onClick={()=>setLiked(l=>({...l,[ct.id]:!l[ct.id]}))} style={{background:"none",border:`1px solid ${liked[ct.id]?"#FF3CAC":"#1C1C24"}`,color:liked[ct.id]?"#FF3CAC":"#555",borderRadius:8,padding:"5px 14px",fontFamily:"'DM Mono'",fontSize:10,cursor:"pointer",transition:"all .2s"}}>{liked[ct.id]?"❤️ LIKED":"🤍 LIKE"}</button>
                  <button style={{background:"none",border:"1px solid #1C1C24",color:"#555",borderRadius:8,padding:"5px 14px",fontFamily:"'DM Mono'",fontSize:10,cursor:"pointer"}} onClick={()=>navigator.clipboard?.writeText(window.location.href).catch(()=>{})}>🔗 SHARE</button>
                </div>
              </div>
            ) : (
              <div style={{background:"linear-gradient(135deg,#0C0C14,#0A0A10)",border:"1px solid #1C1C24",borderRadius:20,padding:30,marginBottom:18,textAlign:"center"}}>
                ...
                <div style={{fontSize:10,color:"#555"}}>Tap any song below to start playing</div>
              </div>
            )}
              ...
              {activeTracks.map((track,idx)=>(
                <div key={track.id} className={`track-row${ct?.id===track.id?" active":""}`} onClick={()=>playTrack(track)}>
                  <div style={{width:28,textAlign:"center",fontFamily:"'Anton'",fontSize:13,color:ct?.id===track.id?"#FFD700":"#333",flexShrink:0}}>
                    {ct?.id===track.id && playing ? <span style={{animation:"spin 1s linear infinite",display:"inline-block"}}>◎</span> : idx+1}
                  </div>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:9,color:"#444",letterSpacing:1}}>{track.genre} · {track.plays||0} plays</div>
                  </div>
                  <div style={{fontSize:10,color:"#444",flexShrink:0}}>{fmtTime(track.duration)}</div>
            ...
            {!adminUnlocked?(
              ...
              <div>
                ...
                <div style={{display:"flex",gap:5,marginBottom:16,flexWrap:"wrap"}}>
                  {[["music","🎵 Music"],["links","🔗 Links"],["drops","💿 Drops"],["settings","🎤 Artist"],["release","⏳ Release"]].map(([id,label])=>(
